...
function validateCatalogUrl(value: unknown, field: "preview" | "zip" | "catalog"): string {
  ...
  if (field === "preview") {
    if (url.hostname !== "nekodrift.app" || !url.pathname.startsWith("/pets/")) throw new Error("Preview URL host/path is not allowed.");
  } else if (field === "catalog") {
    if (url.hostname !== "nekodrift.app" || !url.pathname.startsWith("/pets/catalog.v3/")) throw new Error("Catalog URL host/path is not allowed.");
  } else if (url.hostname !== "zip.nekodrift.app" || !url.pathname.startsWith("/pets/")) {
