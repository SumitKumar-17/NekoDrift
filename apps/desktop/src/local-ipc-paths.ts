...
export function getIpcEndpointConfig(): IpcEndpointConfig {
  const bindEnv = process.env.NEKODRIFT_IPC_BIND;
  const endpointEnv = process.env.NEKODRIFT_IPC_ENDPOINT;
  ...
  if (bindEnv && endpointEnv) {
    ...
    if (bindParsed.kind !== "tcp") {
      throw new Error("NEKODRIFT_IPC_BIND only supports TCP endpoints (e.g., tcp://0.0.0.0:37645 or tcp://127.0.0.1:37645).");
    }
    ...
    if (advertisedParsed.kind !== "tcp") {
      throw new Error("NEKODRIFT_IPC_ENDPOINT must be a TCP endpoint when used with NEKODRIFT_IPC_BIND.");
    }
    // Advertised endpoint must have a concrete host (not 0.0.0.0)
    if (advertisedParsed.host === "0.0.0.0") {
      throw new Error("NEKODRIFT_IPC_ENDPOINT must specify a concrete IPv4 address (not 0.0.0.0) when used as the advertised endpoint.");
    }
    validateAdvertisedHost(advertisedParsed.host);
    if (bindParsed.port !== 0 && advertisedParsed.port !== bindParsed.port) {
      throw new Error("NEKODRIFT_IPC_ENDPOINT must use the same port as NEKODRIFT_IPC_BIND unless NEKODRIFT_IPC_BIND uses port 0.");
    }
  ...
  if (bindEnv) {
    ...
    if (bindParsed.host === "0.0.0.0") {
      throw new Error("When NEKODRIFT_IPC_BIND is set to 0.0.0.0, you must also set NEKODRIFT_IPC_ENDPOINT to specify the advertised endpoint (e.g., the WSL host IP).");
    }
  ...
  if (endpointEnv) {
    throw new Error("NEKODRIFT_IPC_ENDPOINT only controls the advertised discovery endpoint. Set NEKODRIFT_IPC_BIND to opt into TCP IPC listening.");
  }

  // Case 4: No env vars - use platform default
  if (process.platform === "win32") {
    const pipePath = `\\\\.\\pipe\\nekodrift-${randomEndpointPart()}-${process.pid}`;
  ...
  const socketPath = join(runtimeDir, `nekodrift-${process.pid}.sock`);
...
export function parseIpcEndpoint(endpoint: string, options: { readonly allowPortZero?: boolean; readonly allowNonLoopback?: boolean } = {}): IpcEndpoint {
  if (endpoint.length < 1 || endpoint.length > 240) throw new Error("NekoDrift IPC endpoint length is invalid.");
  if (endpoint.includes("\0")) throw new Error("NekoDrift IPC endpoint contains NUL.");
  ...
  if (endpoint.startsWith("tcp://")) {
    ...
    try {
      ...
      throw new Error("NekoDrift TCP IPC endpoint is invalid.");
    }

    if (url.protocol !== "tcp:" || url.username || url.password || (url.pathname !== "" && url.pathname !== "/") || url.search || url.hash) {
      throw new Error("NekoDrift TCP IPC endpoint must be tcp://<host>:<port> with no credentials, path, query, or fragment.");
    }

    const host = url.hostname;
    if (!isValidIpv4(host)) {
      throw new Error("NekoDrift TCP IPC endpoint host must be a valid IPv4 address (127.0.0.1, 0.0.0.0, or a concrete address).");
    }

    if (!options.allowNonLoopback && host !== "127.0.0.1") {
      throw new Error("NekoDrift TCP IPC endpoint must bind to loopback host 127.0.0.1.");
    }

    const port = Number(url.port);
    const minPort = options.allowPortZero ? 0 : 1;
    if (!Number.isInteger(port) || port < minPort || port > 65_535 || String(port) !== url.port) {
      throw new Error("NekoDrift TCP IPC endpoint port is invalid.");
    }
...
function validateBindHost(host: string): void {
  ...
  if (!isPrivateOrLocalIpv4(host)) {
    throw new Error(`NEKODRIFT_IPC_BIND host ${host} is not a private/local IPv4 address. Only 0.0.0.0, loopback (127.0.0.1), private (10.x.x.x, 172.16-31.x.x, 192.168.x.x), or link-local (169.254.x.x) addresses are allowed.`);
  }
}
...
export function writeDiscoveryFile(endpoint: string, token: string): NekoDriftDiscoveryFile {
  const discovery: NekoDriftDiscoveryFile = {
...
export function removeDiscoveryFile(discovery: NekoDriftDiscoveryFile | null): void {
  if (!discovery) return;
  const path = getDiscoveryFilePath();
  try {
    const current = JSON.parse(readFileSync(path, "utf8")) as Partial<NekoDriftDiscoveryFile>;
...
function getSocketRuntimeDir(): string {
  ...
  if (xdg) {
    return join(xdg, "nekodrift");
  }

  return join("/tmp", `nekodrift-${getUserIdForPath()}`);
}
...
function ensurePrivateRuntimeDir(dir: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`NekoDrift IPC runtime path is not a safe directory: ${dir}`);
  }

  if (typeof process.getuid === "function" && stat.uid !== process.getuid()) {
    throw new Error(`NekoDrift IPC runtime directory is not owned by the current user: ${dir}`);
  }

  try { chmodSync(dir, 0o700); } catch { /* best effort */ }
  const updated = lstatSync(dir);
  if ((updated.mode & 0o777) !== 0o700) {
    throw new Error(`NekoDrift IPC runtime directory is not private: ${dir}`);
  }
}
