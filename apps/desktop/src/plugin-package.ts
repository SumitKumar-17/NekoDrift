...
import { canonicalizePluginPermissions, NEKODRIFT_PLUGIN_MANIFEST_FILENAME, validatePluginManifest, type NekoDriftPluginManifest } from "./plugin-manifest.js";

const maxZipBytes = 5 * 1024 * 1024;
const maxPluginEntryBytes = 1024 * 1024;
const maxPluginZipEntries = 2;
const maxPluginZipUncompressedBytes = defaultMaxPluginManifestBytes + maxPluginEntryBytes;
export type PluginPackageInstall = { readonly manifest: NekoDriftPluginManifest; readonly installPath: string; readonly manifestPath: string; readonly entryPath?: string };
...
export function validatePluginZipUrl(value: string): void { const url = new URL(value); if (url.protocol !== "https:" || url.hostname !== "zip.nekodrift.app" || !url.pathname.startsWith("/plugins/") || url.username || url.password || url.port) throw new Error("Plugin ZIP URL is not allowed."); }

export async function readCatalogPluginManifestFromZip(options: { readonly catalogEntry: AnyPluginCatalogEntry; readonly zip: Buffer; readonly maxManifestBytes?: number; readonly maxEntryBytes?: number }): Promise<{ readonly manifest: NekoDriftPluginManifest; readonly manifestText: string; readonly entryText?: string }> {
  ...
  const text = files.get(NEKODRIFT_PLUGIN_MANIFEST_FILENAME)?.toString("utf8");
  if (text === undefined) throw new Error("Plugin ZIP must contain nekodrift.plugin.json.");
...
export async function installCatalogPluginPackage(options: { readonly userDataPath: string; readonly catalogEntry: AnyPluginCatalogEntry; readonly zip: Buffer; readonly maxManifestBytes?: number }): Promise<PluginPackageInstall> {
  const { manifest, manifestText: text, entryText } = await readCatalogPluginManifestFromZip(options);
  const root = join(options.userDataPath, "plugins"); await fs.mkdir(root, { recursive: true }); await assertDir(root);
  const installPath = join(root, manifest.id); const manifestPath = join(installPath, NEKODRIFT_PLUGIN_MANIFEST_FILENAME); const tempPath = join(root, `.tmp-${manifest.id}-${process.pid}-${Date.now()}`);
  const entryPath = manifest.manifestVersion === 2 ? join(installPath, manifest.entry) : undefined;
  try {
    await fs.rm(tempPath, { recursive: true, force: true });
    await fs.mkdir(tempPath, { recursive: false, mode: 0o700 });
    await assertDir(tempPath);
    await fs.writeFile(join(tempPath, NEKODRIFT_PLUGIN_MANIFEST_FILENAME), text, { mode: 0o600 });
    if (manifest.manifestVersion === 2 && entryPath && entryText !== undefined) {
    ...
    await readSafePluginManifest({ installPath: tempPath, manifestPath: join(tempPath, NEKODRIFT_PLUGIN_MANIFEST_FILENAME), allowedPluginRoots: [root], maxManifestBytes: options.maxManifestBytes, expectedId: manifest.id, expectedVersion: manifest.version });
