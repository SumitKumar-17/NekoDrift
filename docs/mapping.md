# NekoDrift reaction, speech, and Claude hook mapping

This document explains how NekoDrift decides which pet reaction is shown, when the pet says something, and how Claude Code hooks and OpenCode plugin events map to reactions/messages.

## Important terms

- **Reaction**: one of the allowed NekoDrift reaction names, for example `thinking`, `success`, or `error`.
- **Speech/message**: a short text bubble sent with `nekodrift_say` or generated by a Claude hook.
...
NekoDrift currently accepts these reactions:
...
The universal Codex/NekoDrift spritesheet has these animation rows:
...
| --- | --- |
| Reaction only: `nekodrift_react({ reaction: "testing" })` | A stable randomized status line from the `testing` message pool, for example `Running the checks`. |
| Message only: `nekodrift_say({ message: "Done" })` | `Done` |
| Message + reaction: `nekodrift_say({ message: "Done", reaction: "success" })` | `Done` |
...
When NekoDrift Claude hooks are installed, these Claude Code hook events are registered:
...
For `PreToolUse` with `tool_name: "Bash"`, NekoDrift classifies the command as `testing` if the command contains one of these patterns:
...
If speech is throttled, NekoDrift still sends the reaction with `pet.react`. Example: if `Stop` happens twice within 20 seconds, the second one sends `success` as a reaction only instead of saying another success phrase.
...
When NekoDrift OpenCode setup is installed, OpenCode loads the `@neko-drift/opencode` plugin. Plugin hooks return immediately and schedule NekoDrift IPC calls in the background so OpenCode is not blocked by pet activity.
...
| Internal/self NekoDrift tools | NekoDrift MCP/status/say/react calls. | None. | No. |
...
| --- | --- | --- |
| `nekodrift_status` | none | Checks whether NekoDrift is running and which pet is targeted. Does not change pet state. |
| `nekodrift_react` | `{ reaction }` | Drives the reaction animation and may show a short randomized temporary bubble from that reaction's message pool. |
| `nekodrift_say` | `{ message, reaction? }` | Sends a message bubble, optionally with a reaction. The message is what appears in the bubble. |

### MCP message validation

`nekodrift_say` messages must be:
...
NekoDrift decides which pet gets the reaction/message as follows:
