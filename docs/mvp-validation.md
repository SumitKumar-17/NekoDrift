# NekoDrift MVP validation

This is the authoritative local MVP closeout checklist for NekoDrift 2.0.
...
| First-run onboarding | Ready for macOS local validation | Can be retested by removing NekoDrift app data. |
...
| macOS app output | `apps/desktop/dist-electron/mac-arm64/NekoDrift.app` on Apple Silicon |
...
The macOS package uses an NekoDrift MVP `.icns` derived from the current 64×64 tray icon. It may look soft at large sizes. Higher-resolution branding and Windows/Linux icon assets remain future polish.
...
1. Launch the generated packaged app from `apps/desktop/dist-electron/`.
   - Expected: NekoDrift starts without using the dev Electron entrypoint.
...
4. Complete onboarding if it appears.
   ...
   - To retest first-run onboarding, remove the NekoDrift app data directory for your platform before launching again.
5. Open Pet Manager.
...
8. While the packaged app is running, run the local IPC smoke command from the workspace root:
   ...
   ```bash
   pnpm --filter @neko-drift/client smoke:status
   ```
...
9. Open Agent Setup and inspect the Claude preview without applying changes.
   - Expected: Packaged previews use `node` commands pointing inside the packaged app, not unpublished `npx -y @neko-drift/*` commands.
   - Expected MCP shape: `node .../app.asar.unpacked/node_modules/@neko-drift/mcp/dist/index.js`.
   - Expected hooks shape: `node .../app.asar.unpacked/node_modules/@neko-drift/claude/dist/cli.js hook --nekodrift-managed`.
10. Optional mutating Claude validation: configure/install Claude MCP/hooks only if you intentionally want to modify your real Claude settings.
    - Expected: NekoDrift shows what it will change and creates backups before editing.
    - Cleanup: use Agent Setup remove/uninstall actions when done if you do not want to keep the integration.
11. Quit NekoDrift from the tray.
    - Expected: The app exits cleanly and removes transient local IPC discovery state.

## Pet and chat bubble quality checks

While NekoDrift is running, trigger a speech bubble from the workspace root:

```bash
pnpm --filter @neko-drift/client smoke:say "Working on it" thinking
```
...
```bash
pnpm --filter @neko-drift/client smoke:react success
```
...
Packaged Claude integration uses bundled commands inside the packaged NekoDrift app instead of unpublished package names.

Expected command resources live under `app.asar.unpacked`, not `app.asar`, because Claude launches them through external `node`:

```text
.../NekoDrift.app/Contents/Resources/app.asar.unpacked/node_modules/@neko-drift/mcp/dist/index.js
.../NekoDrift.app/Contents/Resources/app.asar.unpacked/node_modules/@neko-drift/claude/dist/cli.js
```

Packaged bundled Claude commands require `node` to be available on Claude's PATH.

Claude settings may contain absolute paths into the packaged NekoDrift app. Moving, deleting, or replacing the app may require opening Agent Setup and using Replace/Install again. Use Agent Setup remove/uninstall before deleting the app if you want to remove NekoDrift-managed Claude entries.
...
- Running `pnpm --filter @neko-drift/client smoke:status` against the local packaged app.
...
- Deleting NekoDrift app data to retest onboarding.

Only run mutating steps intentionally. Automated tests must use temp fixtures and must not edit real Claude settings or real NekoDrift user data.

## Cleanup and data-loss notes

NekoDrift stores normal app state in Electron's per-user app data directory. The app logs the exact state file path on startup:

```text
NekoDrift state initialized at <path>/nekodrift-state.json.
```
...
Claude configuration is separate from NekoDrift app data. Uninstalling or deleting NekoDrift does not automatically remove Claude MCP entries or Claude hooks.
...
- Published `npx -y @neko-drift/*` package flows remain future distribution work; local packaged Claude setup uses bundled app paths.
