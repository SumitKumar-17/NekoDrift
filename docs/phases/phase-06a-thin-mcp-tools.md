# Phase 06A: Thin `@neko-drift/mcp` tools over local IPC

## Goal

Make NekoDrift minimally usable from MCP-capable coding agents through a thin Node/npm/npx MCP server package over the existing Phase 05 local IPC methods.

This sub-phase turns the Phase 05 local IPC control plane into the first agent-facing integration:

```text
MCP-capable agent → @neko-drift/mcp stdio server → @neko-drift/client → desktop local IPC → visible pet
```
...
After this phase, a developer can run the local `@neko-drift/mcp` package as a stdio MCP server, call:
...
and see the running NekoDrift desktop pet respond through local IPC.

If NekoDrift is not running, the MCP server remains alive and `nekodrift_status` reports a clear degraded/unavailable status instead of crashing.
...
- `@neko-drift/mcp` builds and runs with Node/npm/pnpm and is shaped for future `npx -y @neko-drift/mcp` usage.
...
- MCP server exposes exactly these initial tools:
  ...
  - `nekodrift_say`
  - `nekodrift_react`
- `nekodrift_status` calls `@neko-drift/client.status()` and reports reachable/degraded app state as both human-readable text and structured content where supported.
- `nekodrift_say` calls `@neko-drift/client.say()` with the user-provided short message and optional reaction.
- `nekodrift_react` calls `@neko-drift/client.react()` with one allowed reaction.
- MCP tool schemas enforce the public reaction list from `task.txt`.
- MCP tool schemas keep `nekodrift_say.message` short enough for Phase 05 IPC limits and speech safety.
- Tool errors return MCP tool errors (`isError: true`) rather than crashing the server.
- If desktop app/discovery/local IPC is unavailable, `nekodrift_status` reports degraded state clearly.
- If desktop app/discovery/local IPC is unavailable, `nekodrift_say` and `nekodrift_react` return clear tool errors.
...
- `nekodrift_status` clearly distinguishes `configuredPetId` from `actualTargetPetId`.
...
- `@neko-drift/client`, `@modelcontextprotocol/sdk`, and the schema validation library are runtime dependencies when used by the MCP server.
- Create MCP server with name `neko-drift` / package name `@neko-drift/mcp`.
...
- If `--pet` is provided, `nekodrift_status` must explicitly say explicit pet routing is not implemented in Phase 06A and the actual target remains the desktop default pet.

### Degraded behavior

`@neko-drift/client.status()` already returns a typed unavailable result when discovery/connect fails. MCP should preserve this behavior in `nekodrift_status`.

For `say`/`react`, return `isError: true` with a concise message such as:

```text
NekoDrift desktop app is not running or local IPC is unavailable. Open NekoDrift and try again.
```

### Lifecycle

- Handle `SIGINT`/`SIGTERM` by closing MCP server/transport where supported.
- Keep server alive even if NekoDrift desktop app is unavailable.
- Do not implement public `nekodrift_release`.
...
### Risk: agents displaying private content through `nekodrift_say`
...
- `nekodrift_say` is for short status/personality messages only.
...
```bash
pnpm check
pnpm --filter @neko-drift/mcp check
pnpm --filter @neko-drift/mcp build
```
...
- `nekodrift_say` validation rejects overlong/multiline/code/secret/path/URL-like messages.
...
- MCP contract check starts the stdio server and verifies `tools/list` returns exactly `nekodrift_status`, `nekodrift_say`, and `nekodrift_react`.
...
4. Call `nekodrift_status` and confirm it reports NekoDrift reachable/default pet info.

5. Call `nekodrift_react` with `testing` and confirm the visible pet reacts.

6. Call `nekodrift_say` with `Working on it` and `working` and confirm the visible pet message appears.

7. Quit NekoDrift and call `nekodrift_status` again; confirm it reports degraded/unavailable clearly while MCP remains alive.

Manual acceptance question:

```text
Does Phase 06A pass on your machine: @neko-drift/mcp runs over stdio, exposes status/say/react, talks to the running desktop app through local IPC, reports configured `--pet` as not-yet-routed, and reports degraded status when NekoDrift is unavailable?
```
...
- Fixed: Required runtime dependencies for `@neko-drift/client`, MCP SDK, and schema validation library.
...
- Deferred: Auto-launch of installed NekoDrift app is explicitly left for Phase 06B or packaging phase.
...
- Fixed: MCP package `check` builds `@neko-drift/client` first so package-local checks do not depend on stale client `dist` output.
- Fixed: Added stdio smoke contract coverage using the built `dist/index.js`, verifying tool list and degraded `nekodrift_status` with a missing discovery file.
- Fixed: Unavailable `nekodrift_status` human text now still explains configured `--pet` and Phase 06A deferred routing.
