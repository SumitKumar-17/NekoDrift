...
```text
@neko-drift/mcp --pet snoopy starts
...
- Auto-launching installed NekoDrift desktop app from MCP.
...
- Running `@neko-drift/mcp` without `--pet` acquires a lease for the desktop default pet and tools affect the default pet.
- Running `@neko-drift/mcp --pet <installed-non-default-id>` opens that installed pet as a temporary agent pet and tools affect that pet.
...
- If `--pet` is missing or invalid/uninstalled, desktop falls back to the default pet and `nekodrift_status` reports the fallback clearly.
...
- `@neko-drift/client` exposes typed helpers for acquiring, heartbeating, releasing, status, say, and react with optional lease id.
- `@neko-drift/mcp` acquires a lease during startup after MCP server initialization, not on first tool call.
- `@neko-drift/mcp --pet <id>` sends requested pet id to `lease.acquire`.
- `@neko-drift/mcp` accepts a bounded raw `--pet` string and lets desktop validate/fallback; only values that exceed length/control-character limits are fatal CLI errors.
...
- `nekodrift_say` and `nekodrift_react` target the acquired lease's actual pet.
- If lease acquisition fails because NekoDrift is unavailable, MCP remains alive in degraded mode and `nekodrift_status` reports unavailable clearly.
- In degraded mode, `nekodrift_say` and `nekodrift_react` return MCP tool errors without crashing the server.
...
- Public MCP tool set remains exactly `nekodrift_status`, `nekodrift_say`, `nekodrift_react`.
...
- `nekodrift_status` reports lease/degraded state.
- `nekodrift_say`/`nekodrift_react` require an active lease to target explicit/default pet; if no lease, return `isError: true` with clear unavailable guidance.
...
Auto-launch of installed NekoDrift desktop app remains deferred. It is important product behavior, but packaging/app discovery is not mature enough yet. Phase 06B focuses on lease lifecycle when the app is already running.
...
- `nekodrift_status` reports requested/configured and actual target.
...
4. Call `nekodrift_status`; confirm it reports the current default pet as actual target and `routingImplemented: true`.

5. Call `nekodrift_react` / `nekodrift_say`; confirm the default pet reacts/says.
...
8. Call `nekodrift_react` / `nekodrift_say`; confirm Snoopy reacts/says, not the default pet.
...
- Fixed: `@neko-drift/mcp` should accept bounded raw `--pet` strings and let desktop validate/fallback, so invalid/uninstalled requested pets can follow product fallback behavior.
