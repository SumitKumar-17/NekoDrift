...
- Settings opens as a polished light NekoDrift window instead of the older dark generic card layout.
...
- Agent setup window title/user-facing definition uses **Integrations** (`NekoDrift — Integrations`) while preserving internal `agent-setup` identifiers.
...
- Settings-specific light styles are scoped to `body[data-nekodrift-view="settings"]`; avoid broad edits to global `button`, `.card`, or `.setting-row` behavior.
...
2. Add scoped CSS under `body[data-nekodrift-view="settings"]` to avoid broad regressions.
...
2. Open Settings from the tray and confirm it matches the light NekoDrift polish direction.
...
- Added a Launch NekoDrift at login toggle backed by Electron login item settings on supported platforms.
