...
- Import copies only validated `pet.json` metadata and `spritesheet.webp` into NekoDrift user data under the normal installed-pets directory.
...
- Destination path must stay inside NekoDrift pets storage.
- Import writes fresh regular files into NekoDrift storage rather than copying links.
...
7. Confirm removing it deletes the NekoDrift copy, not the original Codex folder.
