...
Add full NekoDrift support for OpenCode with parity to the current Claude Code integration, without over-splitting implementation work.

Target parity means:

- NekoDrift MCP tools available in OpenCode.
- NekoDrift instructions installed into OpenCode instructions.
...
1. **MCP tools**
   - Public tools stay exactly `nekodrift_status`, `nekodrift_say`, `nekodrift_react`.
   - MCP server: `packages/mcp/src/server.ts`.
   - CLI wrapper: `packages/cli/src/index.ts` → `nekodrift mcp --pet <id>`.
...
2. **Instructions/memory**
   - Claude writes `~/.claude/nekodrift.md` and imports it from `~/.claude/CLAUDE.md`.
...
4. **CLI project setup**
   - `nekodrift configure --agent claude --pet <id> --cwd <project>` configures project-local Claude MCP and hooks.
...
- **MCP config**
  ...
  - `opencode mcp add` is interactive, so automated NekoDrift setup should directly edit JSON/JSONC config with the same shape.
...
- **Plugins/events**
  ...
  - OpenCode awaits plugin hooks directly, so plugin hooks must return immediately and run NekoDrift calls fire-and-forget.
...
- NekoDrift calls run fire-and-forget with internal `.catch()` and debug-only logging.
...
- Plugin hook functions return without awaiting NekoDrift IPC.
...
**Goal:** Extend `nekodrift configure` to support project-local OpenCode setup.
...
- Write project config entries for:
  - `mcp.nekodrift`;
  - NekoDrift instructions file, e.g. `.opencode/nekodrift.md`;
  - NekoDrift plugin spec/options.
...
- `nekodrift configure --agent opencode --pet fixer --local-dev --cwd <tmp>` writes expected project config offline.
- Re-running is idempotent.
- `--force` replaces only NekoDrift-managed entries.
...
- Global setup can:
  ...
  - install/update global NekoDrift instructions;
  - install/update/remove global NekoDrift plugin;
...
- Remove only removes NekoDrift-managed entries.
...
- Plugin hooks are awaited by OpenCode; NekoDrift work must be fire-and-forget, not merely timeout-bounded.
...
- **Fixed:** Plugin hooks must return immediately and run NekoDrift calls fire-and-forget.
...
- `packages/cli` supports `nekodrift configure --agent opencode` for project-local setup.
