...
Add the OpenCode runtime plugin that turns OpenCode activity into NekoDrift reactions and short safe speech.

This phase builds on Phase 19A's foundation package and shared speech validation. It should make the plugin importable/testable and ready for manual OpenCode config, but it should not add Desktop UI or `nekodrift configure --agent opencode` yet.
...
Developers can manually add the plugin to an OpenCode config and see NekoDrift react to OpenCode activity.
...
- `@neko-drift/opencode` exposes an OpenCode-compatible server plugin entrypoint.
- Package export contract supports OpenCode's loader expectations:
  ...
  - package-level resolution of `@neko-drift/opencode/server` is covered by tests.
- Plugin options accept:
  - optional `pet` id using the same strict pet id validation as Claude/OpenCode foundation;
  - optional debug flag or rely on `NEKODRIFT_DEBUG=1`.
- Plugin hooks must return immediately. NekoDrift IPC work must run fire-and-forget and never be awaited by OpenCode plugin hooks.
...
- NekoDrift calls must be best-effort:
  - catch all errors internally;
  - debug-only sanitized logging;
  - short client timeouts;
  - never throw back into OpenCode.
- If `pet` is configured, plugin reactions/speech should acquire or reuse an NekoDrift lease and pass `leaseId` to `react`/`say`.
...
- Event mapping must cover:
  ...
  - other non-NekoDrift tools → `working`.
...
- The plugin must ignore NekoDrift MCP tool calls to avoid self-reaction loops, including likely OpenCode MCP tool names such as:
  - `nekodrift_nekodrift_status`
  - `nekodrift_nekodrift_say`
  - `nekodrift_nekodrift_react`
- Speech must use shared `@neko-drift/agent-events` validation and message pools.
...
Therefore every plugin hook must schedule NekoDrift work and then return immediately. Do not `await client.react`, `await client.say`, or `await client.acquireLease` directly inside a hook.
...
### NekoDrift client behavior

Use `createNekoDriftClient({ connectTimeoutMs: 500, responseTimeoutMs: 500 })` or a similarly short timeout.
...
- `classifyOpenCodeToolReaction(toolName, args): NekoDriftReaction | undefined`
- `classifyOpenCodeBusEvent(event): { reaction?, speechCategory?, forceSpeech? } | undefined`
- `shouldIgnoreNekoDriftTool(toolName): boolean`
...
- NekoDrift MCP tools: suppress names ending in or equal to `nekodrift_status`, `nekodrift_say`, `nekodrift_react`, including server-prefixed forms.
...
Use shared messages and validator from `@neko-drift/agent-events`.
...
- Adding runtime client dependency to `@neko-drift/opencode` is expected in this phase; avoid depending on CLI or desktop.
...
- Plugin must ignore NekoDrift MCP tools to avoid loops.
- Plugin must catch all NekoDrift client errors.
...
- Dynamic import of package export `@neko-drift/opencode/server` returns default object with `id` and `server()`.
...
- NekoDrift client errors are swallowed.
...
- NekoDrift MCP tools are ignored.
...
2. Run `pnpm check`.
3. Start NekoDrift desktop locally.
4. Build packages.
...
11. Confirm NekoDrift MCP tool calls do not trigger recursive reactions.
...
- **Fixed:** Added explicit `@neko-drift/opencode/server` export requirement and package-level import test.
