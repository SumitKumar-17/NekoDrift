...
Add first-class NekoDrift support for Pi through a dedicated Pi extension package, with desktop and docs visibility, while keeping the runtime local, safe, and non-blocking.

Target support means:

- Pi agent activity drives NekoDrift reactions automatically.
- Pi users can install the NekoDrift Pi extension with Pi's package system. The NekoDrift desktop app remains a separate app installation.
- Pi users can run a small `/nekodrift` command namespace for status and manual checks.
- NekoDrift desktop can show Pi in Integrations using `apps/desktop/assets/integrations/pi.svg`.
...
## Current NekoDrift integration model to mirror

NekoDrift currently has three integration layers:
...
3. **Generic fallback**
   - `packages/mcp` exposes `nekodrift_status`, `nekodrift_react`, and `nekodrift_say` for stdio MCP clients.
...
- Extensions can use `ctx.ui.notify()`, `ctx.ui.setStatus()`, widgets, and custom TUI pieces, but NekoDrift should not require custom Pi UI for the first release.
- Pi extensions run with full local system permissions, so NekoDrift docs must treat package trust as a security boundary.
...
The package should export one Pi extension that uses `@neko-drift/client` to send local IPC updates. It should not depend on MCP for core behavior.
...
```json
{
  ...
  "license": "MIT",
  "type": "module",
  "keywords": ["pi-package", "nekodrift", "coding-agent"],
  "main": "./dist/index.js",
  ...
  "repository": {
    "type": "git",
    "url": "git+https://github.com/alvinunreal/nekodrift.git",
...
- The extension should tolerate missing NekoDrift desktop IPC and keep Pi startup/session flow unaffected.
- Add `@neko-drift/pi` to `scripts/release-npm.mjs` publish order after its dependencies.
...
- Reuse `@neko-drift/agent-events` for shared event messages/safety if it fits the exact validation requirements; otherwise document the reason and test Pi against the same rejection corpus as desktop/MCP.

## Runtime behavior

The extension should:

- Create a small NekoDrift runtime wrapper around `@neko-drift/client`.
- Cache connection status only briefly; do not assume the desktop app stays available.
- Use short timeouts for NekoDrift calls.
- Run automatic event updates fire-and-forget.
- Swallow NekoDrift IPC failures by default, with optional debug logging.
...
| Pi event | Condition | NekoDrift reaction | Speech |
| --- | --- | --- | --- |
...
- Ignore NekoDrift-related commands/tools to prevent self-trigger loops.
...
| Command | Behavior |
| --- | --- |
| `/nekodrift status` | Check whether the NekoDrift desktop app is reachable and which pet is targeted. |
| `/nekodrift test` | Send a short safe test message and `waving` or `success` reaction. |
| `/nekodrift react <reaction>` | Set one allowed reaction. |
| `/nekodrift say <message>` | Show one validated short speech bubble. |
| `/nekodrift help` | Show available subcommands. |

Allowed manual reactions should match the public NekoDrift reaction set:

```text
idle, thinking, working, editing, running, testing, waiting, waving, success, error, celebrating
```

Manual speech must use the same validation rules as other NekoDrift surfaces:
...
The Pi integration must preserve the NekoDrift safety model.
...
- Manual `/nekodrift say` input is validated before sending.
- NekoDrift failures never block Pi model calls or tool execution.
...
- Initially mark the card as planned/coming soon unless `@neko-drift/pi` setup is implemented in the desktop app.
- If setup is implemented, expose a simple detail panel with:
  - Pi command detection, if practical;
  - install instructions using `pi install npm:@neko-drift/pi`;
  - project-local install instructions using `pi install -l npm:@neko-drift/pi`;
  - status/check guidance;
  - remove guidance using `pi remove npm:@neko-drift/pi`.
...
- NekoDrift desktop must be running for pet updates.
...
When `@neko-drift/pi` is actually published and supported, update the frontmatter from planned/inactive to active/supported and link it from active integration surfaces.

## Non-goals

- Do not modify Pi upstream.
- Do not require MCP for Pi's automatic reactions.
- Do not add new public NekoDrift MCP tools.
- Do not expose pet install/remove/default controls to the Pi model in MVP.
- Do not expose model-callable NekoDrift tools through Pi in MVP.
...
- Do not make NekoDrift availability affect Pi startup, model requests, or tool execution.
...
- Add workspace package `@neko-drift/pi`.
...
- Add a small NekoDrift client wrapper with timeout and failure swallowing.
- Use explicit short timeouts for NekoDrift calls, initially 500ms for automatic reactions.
...
- Register `/nekodrift` command with `status`, `test`, `react`, `say`, and `help` subcommands.
...
- Extension factory returns quickly and does not require NekoDrift desktop to be running.
- Automatic event handlers do not await IPC in a way that blocks Pi.
- `/nekodrift say` rejects unsafe text.
...
- `pnpm --filter @neko-drift/pi check`
- `pnpm --filter @neko-drift/client check`
- `pnpm --filter @neko-drift/agent-events check`
- `pnpm --filter @neko-drift/pi pack --dry-run`
...
- Confirm that unavailable NekoDrift desktop app does not produce noisy failures.
...
- Pi can load the local NekoDrift extension.
- Pi can load the packed package locally.
- `/nekodrift status` reports reachable/unreachable clearly.
- Tool start/end events produce expected reactions when NekoDrift is running.
- No prompts, outputs, or commands appear in pet speech during normal automation.
- Pi continues normally when NekoDrift is closed.
- Malformed/missing discovery files fail clearly or are ignored by Pi without NekoDrift-specific noise.

**Checks:**

- `pnpm --filter @neko-drift/pi check`
- Manual Pi smoke test with NekoDrift running.
- Manual Pi smoke test with NekoDrift closed.
