...
Claude Code integration for NekoDrift.
...
- Project-local detection: Checks `.claude/settings.local.json` for `--nekodrift-managed --project-local`
...
- Marker: `--nekodrift-managed` in command for identification
...
- MCP server name: `nekodrift`
- Command modes: `published` (npx -y @neko-drift/mcp), `local`/`bundled` (node path)
...
- Re-exports from `@neko-drift/agent-events`
...
Decision: { reaction?, speechCategory? }
    ↓
hasProjectLocalNekoDriftHook() → Skip if project-local exists
    ↓
...
**Dependencies**:
- `@neko-drift/client` - IPC communication
- `@neko-drift/agent-events` - Speech pools

**External Commands**:
- `claude` - Claude Code CLI for MCP and settings management

**Consumers**:
- `@neko-drift/cli` - `configure` command for Claude projects
...
- `validateNekoDriftPetArg()`, `openPetsHookMarker`, `claudeHookEvents`
