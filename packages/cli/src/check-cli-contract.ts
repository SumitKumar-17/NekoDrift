...
assert.equal(parseConfigureArgs(["--agent", "agent", "--pet", "fixer"]).agent, "agent");
...
try {
  ...
  const agentProject = join(dir, "agent-project");
  mkdirSync(agentProject);
  await configureProject({ agent: "agent", petId: "fixer", cwd: agentProject, yes: true, force: false, localDev: false });
  const agentConfigPath = join(agentProject, ".opencode", "opencode.jsonc");
  const agentInstructionPath = join(agentProject, ".opencode", "nekodrift.md");
  const agentConfig = JSON.parse(readFileSync(agentConfigPath, "utf8")) as { readonly mcp?: Record<string, { readonly command?: readonly string[] }>; readonly instructions?: readonly string[]; readonly plugin?: readonly unknown[] };
  assert.deepEqual(agentConfig.mcp?.nekodrift?.command, ["npx", "-y", `@neko-drift/cli@${packageVersion}`, "mcp", "--pet", "fixer"]);
  assert.deepEqual(opencodeConfig.instructions, [".opencode/nekodrift.md"]);
  assert.deepEqual(agentConfig.plugin, [[`@neko-drift/agent@${packageVersion}`, { pet: "fixer" }]]);
  assert.match(readFileSync(agentInstructionPath, "utf8"), /NEKODRIFT:START/);
  await configureProject({ agent: "agent", petId: "fixer", cwd: agentProject, yes: true, force: false, localDev: false });
  const agentConfigAgain = readFileSync(agentConfigPath, "utf8");
  assert.equal((agentConfigAgain.match(/@neko-drift/agent/g) ?? []).length, 1);

  const agentExistingTop = join(dir, "agent-existing-top");
  mkdirSync(agentExistingTop);
  writeFileSync(join(agentExistingTop, "opencode.json"), JSON.stringify({ theme: "x", mcp: { other: { type: "local", command: ["other"] } }, plugin: ["other-plugin"], instructions: ["README.md"] }, null, 2), "utf8");
  await configureProject({ agent: "agent", petId: "fixer", cwd: agentExistingTop, yes: true, force: false, localDev: true });
  const existingConfig = JSON.parse(readFileSync(join(agentExistingTop, "opencode.json"), "utf8")) as { readonly theme?: string; readonly mcp?: Record<string, { readonly command?: readonly string[] }>; readonly plugin?: readonly unknown[]; readonly instructions?: readonly string[] };
  ...
  const agentLowerOwner = join(dir, "agent-lower-owner");
  mkdirSync(join(agentLowerOwner, ".opencode"), { recursive: true });
  writeFileSync(join(agentLowerOwner, "opencode.json"), JSON.stringify({ theme: "top" }, null, 2), "utf8");
  writeFileSync(join(agentLowerOwner, ".opencode", "opencode.jsonc"), JSON.stringify({ mcp: { nekodrift: { type: "local", command: ["npx", "-y", "@neko-drift/cli@0.0.1", "mcp", "--pet", "helper"], enabled: true } } }, null, 2), "utf8");
  await configureProject({ agent: "agent", petId: "fixer", cwd: agentLowerOwner, yes: true, force: false, localDev: false });
  const lowerTop = readFileSync(join(agentLowerOwner, "opencode.json"), "utf8");
  const lowerOwned = JSON.parse(readFileSync(join(agentLowerOwner, ".opencode", "opencode.jsonc"), "utf8")) as { readonly mcp?: Record<string, { readonly command?: readonly string[] }> };
  ...
  const agentCustomProject = join(dir, "agent-custom");
  mkdirSync(agentCustomProject);
  writeFileSync(join(agentCustomProject, "opencode.json"), JSON.stringify({ mcp: { nekodrift: { type: "local", command: ["my-nekodrift-wrapper"] } } }), "utf8");
  await assert.rejects(() => configureProject({ agent: "agent", petId: "fixer", cwd: agentCustomProject, yes: true, force: false, localDev: false }));
  assert.equal(readFileSync(join(agentCustomProject, "opencode.json"), "utf8").includes("@neko-drift/cli"), false);

  const agentInstructionProject = join(dir, "agent-instruction");
  mkdirSync(join(agentInstructionProject, ".opencode"), { recursive: true });
  writeFileSync(join(agentInstructionProject, ".opencode", "nekodrift.md"), "User text\n", "utf8");
  await configureProject({ agent: "agent", petId: "fixer", cwd: agentInstructionProject, yes: true, force: false, localDev: false });
  const instructionText = readFileSync(join(agentInstructionProject, ".opencode", "nekodrift.md"), "utf8");
  assert.match(instructionText, /User text/);
  assert.match(instructionText, /NEKODRIFT:START/);

  const agentSymlinkProject = join(dir, "agent-symlink");
  const agentOutside = join(dir, "outside-agent");
  mkdirSync(agentSymlinkProject);
  mkdirSync(agentOutside);
  writeFileSync(join(agentOutside, "opencode.jsonc"), "{}\n", "utf8");
  writeFileSync(join(agentOutside, "nekodrift.md"), "outside\n", "utf8");
  symlinkSync(agentOutside, join(agentSymlinkProject, ".opencode"));
  await assert.rejects(() => configureProject({ agent: "agent", petId: "fixer", cwd: agentSymlinkProject, yes: true, force: false, localDev: false }));
