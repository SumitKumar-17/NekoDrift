...
import { createNekoDriftMcpServer } from "./server.js";
import { createMcpStatus, sanitizeUnavailableReason, type NekoDriftMcpStatus } from "./tools.js";
...
const unavailableStatus = createMcpStatus({ ok: false, appRunning: false, unavailableReason: "/Users/alvin/.config/NekoDrift/runtime/ipc.json ENOENT" }, "snoopy");
...
if (sanitizeUnavailableReason("/tmp/nekodrift-501/nekodrift-1.sock ENOENT")?.includes("/tmp")) {
...
async function checkMcpServerContract(): Promise<void> {
  ...
  const server = createNekoDriftMcpServer({ configuredPetId: "snoopy", client: fakeClient, lease: { lease: await fakeClient.acquireLease() }, leaseReady: Promise.resolve() });
  const client = new Client({ name: "nekodrift-contract", version: "0.0.0" });

  await Promise.all([server.connect(serverTransport), client.connect(clientTransport)]);
  try {
    const tools = await client.listTools();
    const names = tools.tools.map((tool) => tool.name).sort();
    if (names.join(",") !== "nekodrift_react,nekodrift_say,nekodrift_status") {
      throw new Error(`Unexpected MCP tool list: ${names.join(",")}`);
    }

    const status = await client.callTool({ name: "nekodrift_status", arguments: {} }, CallToolResultSchema);
    const structured = status.structuredContent as unknown as NekoDriftMcpStatus;
    ...
    const react = await client.callTool({ name: "nekodrift_react", arguments: { reaction: "waving" } }, CallToolResultSchema);
    ...
    const invalidReact = await client.callTool({ name: "nekodrift_react", arguments: { reaction: "bad" } }, CallToolResultSchema);
    if (!invalidReact.isError) throw new Error("Invalid reaction was not rejected.");

    const invalidSay = await client.callTool({ name: "nekodrift_say", arguments: { message: "const secret = 1" } }, CallToolResultSchema);
...
async function checkStdioServerContract(): Promise<void> {
  const transport = new StdioClientTransport({
    ...
    env: { ...process.env, NEKODRIFT_DISCOVERY_FILE: join(process.cwd(), ".missing-nekodrift-discovery.json") },
    stderr: "pipe",
  });
  const client = new Client({ name: "nekodrift-stdio-contract", version: "0.0.0" });
