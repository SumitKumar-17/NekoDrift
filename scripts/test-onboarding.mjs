...
// Clean settings
fs.rmSync(path.join(process.env.HOME, '.config/comnyang'), { recursive: true, force: true });
...
// Screenshot step 1
await onboarding.screenshot({ path: '/tmp/comnyang-shots/step1-name.png' });
...
// Screenshot step 2
await onboarding.screenshot({ path: '/tmp/comnyang-shots/step2-color.png' });
...
await onboarding.screenshot({ path: '/tmp/comnyang-shots/step3-done.png' });
console.log('step3 screenshot OK');

// Cat window screenshot
await catWin.screenshot({ path: '/tmp/comnyang-shots/cat-window.png' });
...
if (finalCat) {
  await finalCat.screenshot({ path: '/tmp/comnyang-shots/cat-after-onboard.png' });
